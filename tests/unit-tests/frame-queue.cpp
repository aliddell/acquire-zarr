...
test_basic_operations()
{
    ...
    // Pushing
    CHECK(queue.push(frame, "foo", std::nullopt, std::nullopt));
    CHECK(queue.size() == 1);
    CHECK(!queue.empty());
    ...
    CHECK(queue.pop(received_frame, received_key, frame_id, timestamp));
...
test_capacity()
{
    ...
    for (size_t i = 0; i < capacity; ++i) {
        ...
        bool result =
          queue.push(frame, std::to_string(i), std::nullopt, std::nullopt);
        CHECK(result);
    }

    // Queue should be full (next push should fail)
    zarr::LockedBuffer extra_frame(std::move(ByteVector(100)));
    bool push_result = queue.push(
      extra_frame, std::to_string(capacity), std::nullopt, std::nullopt);
    CHECK(!push_result);
    ...
    std::optional<uint64_t> timestamp;
    bool pop_result =
      queue.pop(received_frame, received_key, frame_id, timestamp);
    CHECK(pop_result);
    ...
    push_result =
      queue.push(new_frame, std::to_string(capacity), frame_id, std::nullopt);
    CHECK(push_result);
    CHECK(queue.size() == capacity);
}
...
test_producer_consumer()
{
    ...
    std::thread producer([&queue, n_frames, frame_size]() {
        for (size_t i = 0; i < n_frames; ++i) {
            ...
            while (!queue.push(frame, "spam", std::nullopt, std::nullopt)) {
    ...
    std::thread consumer([&queue, n_frames]() {
        ...
        while (frames_received < n_frames) {
            ...
            std::optional<uint64_t> frame_id;
            std::optional<uint64_t> timestamp;
            if (queue.pop(frame, received_key, frame_id, timestamp)) {
...
test_throughput()
{
    ...
    for (size_t i = 0; i < iterations; ++i) {
        CHECK(queue.push(data, std::to_string(i), std::nullopt, std::nullopt));
        CHECK(queue.pop(received_frame, received_key, frame_id, timestamp));
